...
var styleInject = require('/home/minhna/WORKS/Mike/react-sortable-tree/node_modules/style-inject/dist/style-inject.es.js');
...
class TreeNodeComponent extends React.Component {
    render() {
        ...
        return connectDropTarget(React.createElement("div", { ...otherProps, style: { height: `${calculatedRowHeight}px` }, className: classnames('rst__node', rowDirectionClass ?? ''), ref: (node) => (this.node = node) },
            scaffold,
            React.createElement("div", { className: "rst__nodeContent", style: style }, React.Children.map(children, (child) => React.cloneElement(child, {
                isOver,
                canDrop,
                draggedNode,
            })))));
...
const TreePlaceholder = (props) => {
    ...
    return connectDropTarget(React.createElement("div", null, React.Children.map(children, (child) => React.cloneElement(child, {
        ...otherProps,
    }))));
};
...
class ReactSortableTree extends React.Component {
    ...
    render() {
        ...
        if (rows.length === 0) {
            ...
            list = (React.createElement(virtua.VList, { id: "vlist", ref: this.listRef, dragDropManager: dragDropManager, style: innerStyle, count: rows.length }, (index) => {
...
const SortableTreeWithoutDndContext = (props) => {
    return (React.createElement(reactDnd.DndContext.Consumer, null, ({ dragDropManager }) => dragDropManager === undefined ? undefined : (React.createElement(ReactSortableTree, { ...props, dragDropManager: dragDropManager }))));
};
const SortableTree = (props) => {
    return (React.createElement(reactDnd.DndProvider, { debugMode: props.debugMode, backend: reactDndHtml5Backend.HTML5Backend },
